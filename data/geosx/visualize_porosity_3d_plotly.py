#!/usr/bin/env python3
"""
Script para visualizar mapas de calor 3D de porosidad usando Plotly.
...
def load_porosity(filepath):
    """Carga el archivo de porosidad."""
...
def create_3d_slices_plotly(data_3d, x_slice=None, y_slice=None, z_slice=None,
                            colormap='Viridis', log_scale=False):
    """
    Crea visualización 3D con 3 cortes planos usando Plotly.

    Args:
        data_3d: Array 3D de porosidad
    ...
    if log_scale:
        ...
        data_viz = np.log10(data_3d + 1e-6)
        colorbar_title = 'log10(Porosidad)'
    else:
        data_viz = data_3d
        colorbar_title = 'Porosidad'
    ...
    fig.update_layout(
        title='Mapa de Calor 3D - Porosidad<br>3 Cortes Planos',
        scene=dict(
            xaxis_title='X',
            yaxis_title='Y',
            zaxis_title='Z',
...
def main():
    parser = argparse.ArgumentParser(
        description='Visualiza mapas de calor 3D de porosidad con Plotly (WebGL)'
    )
    parser.add_argument(
        'filepath',
        type=str,
        default='porosity.npy',
        nargs='?',
        help='Ruta al archivo .npy de porosidad'
    )
    parser.add_argument(
        '--x-slice',
        type=int,
    ...
    parser.add_argument(
        ...
        type=str,
        default='Viridis',
        help='Mapa de colores de Plotly (Viridis, Plasma, Cividis, etc.)'
    )
    parser.add_argument(
        '--output',
        type=str,
        default='porosity_3d.html',
        help='Nombre del archivo HTML de salida'
    )
    parser.add_argument(
        '--log',
        action='store_true',
        help='Usar escala logarítmica (útil para rangos grandes)'
    )
    parser.add_argument(
        '--show',
        action='store_true',
    ...
    try:
        # Cargar datos
        print("Cargando datos de porosidad...")
        data = load_porosity(args.filepath)
